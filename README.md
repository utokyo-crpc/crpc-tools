...
- **CRPC固有** — セットアップ用インストーラ、音声文字起こしの起動ラッパ、CRPC専用Skill（`skills/`）、Claude.ai用プロンプト（`prompts/`）。
...
話者名の指定やスライド参照ができる高精度版は Claude Code の `/transcribe-meeting`（詳細は claude-toolkit を参照）。
...
├── audio-transcribe.command / audio-transcribe.bat  # 音声文字起こし
